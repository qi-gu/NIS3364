...
from scapy.all import Padding, Raw, hexdump, defragment
...
class merge_IP:
	def __init__(self, packets):
		...
		self.root.title('IP分片重组')

		self.packet_table = None
		self.packets = self.__handle_packets(packets)
	...
	def __handle_packets(self, packets):
		...
		for packet in packets:
			if packet.haslayer("IP"):
				if packet["IP"].frag > 0 or "MF" in packet["IP"].flags:
					id = (packet["IP"].src, packet["IP"].dst, packet["IP"].id)
					if id not in fragments:
						fragments[id] = []
					fragments[id].append(packet)
				else:
					reassembled_packets.append(packet)
			else:
				pass
		
		for fragment_list in fragments.values():
			reassembled_packets += list(defragment(fragment_list))
